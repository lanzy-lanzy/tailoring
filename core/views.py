...
def admin_dashboard(request):
    ...
    today_revenue = (
        Payment.objects.filter(status="completed", payment_date__date=today).aggregate(
            total=Sum("amount")
        )["total"]
        or 0
    )
...
def fabric_create(request):
    ...
    template = (
        "inventory/partials/fabric_form.html"
        if request.headers.get("HX-Request")
...
def fabric_edit(request, pk):
    ...
    template = (
        "inventory/partials/fabric_form.html"
        if request.headers.get("HX-Request")
        else "inventory/fabric_edit.html"
    )
...
def accessory_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'Accessory "{accessory.name}" created successfully.'
            )
    ...
    template = (
        "inventory/partials/accessory_form.html"
        if request.headers.get("HX-Request")
...
def accessory_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'Accessory "{accessory.name}" updated successfully.'
            )
    ...
    template = (
        "inventory/partials/accessory_form.html"
        if request.headers.get("HX-Request")
        else "inventory/accessory_edit.html"
    )
...
def garment_type_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'Garment type "{garment_type.name}" created successfully.'
            )
...
def garment_type_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(
                request, f'Garment type "{garment_type.name}" updated successfully.'
            )
...
def garment_type_remove_accessory(request, pk, accessory_pk):
    ...
    gta = get_object_or_404(
        GarmentTypeAccessory, garment_type_id=pk, accessory_id=accessory_pk
    )
    gta.delete()
        ...
        if create_new_customer:
            ...
            if customer_name and customer_phone:
                customer = Customer.objects.create(
                    name=customer_name, contact_number=customer_phone
                )
        else:
        ...
        try:
            with transaction.atomic():
                ...
                if not tailor:
                    ...
                    tailors = (
                        User.objects.filter(profile__role="tailor")
                        .annotate(
                            task_count=Count(
                                "assigned_tasks",
                                filter=Q(
                ...
                if tailor:
                    task = TailoringTask.objects.create(
                        order=order, tailor=tailor, status="assigned"
                    )
...
def order_detail(request, pk):
    ...
    return render(request, "orders/detail.html", {"order": order, "task": task})
...
def order_edit(request, pk):
    ...
    if order.status in ["completed", "delivered", "cancelled"]:
        messages.error(
            request, f"Cannot edit orders with status: {order.get_status_display()}"
        )
        return redirect("order_detail", pk=pk)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                old_fabric_needed = (
                    order.garment_type.estimated_fabric_meters * old_quantity
                )
                ...
                messages.success(
                    request, f"Order {order.order_number} updated successfully."
                )
                return redirect("order_detail", pk=order.pk)
    else:
...
def task_create(request):
    ...
    if request.method == "POST":
        ...
        with transaction.atomic():
            ...
            task, created = TailoringTask.objects.get_or_create(
                order=order, defaults={"tailor": tailor, "status": "assigned"}
            )
            ...
            if request.headers.get("HX-Request"):
                return HttpResponse(
                    status=204, headers={"HX-Redirect": f"/orders/{order.pk}/"}
                )
            return redirect("order_detail", pk=order.pk)
    ...
    return render(
        request,
        "tasks/create.html",
        {"order": order, "tailors": tailors, "current_task": current_task},
    )
...
def payment_create(request, order_pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if request.headers.get("HX-Request"):
                return HttpResponse(
                    status=204, headers={"HX-Redirect": f"/orders/{order_pk}/"}
                )
            return redirect("order_detail", pk=order_pk)
    else:
        form = PaymentForm(
            initial={"amount": order.remaining_balance, "payment_type": "balance"}
        )

    return render(request, "payments/create.html", {"form": form, "order": order})
...
def send_order_ready_sms(order):
    ...
    if balance > 0:
        balance_info = (
            f"Please bring the remaining balance of P{balance:.2f} upon pickup. "
        )
    else:
...
def reports_dashboard(request):
    ...
    while current_date <= end_date:
        count = Order.objects.filter(order_date__date=current_date).count()
        daily_orders.append({"date": current_date, "count": count})
        current_date += timedelta(days=1)

    # Order status breakdown
    order_status_breakdown = (
        orders_in_range.values("status").annotate(count=Count("id")).order_by("status")
    )

    total_orders_in_range = orders_in_range.count()
    for status in order_status_breakdown:
        status["percentage"] = (
            (status["count"] / total_orders_in_range * 100)
            if total_orders_in_range > 0
            else 0
        )

    # ===== REVENUE & FINANCIAL ANALYTICS =====
    # Total revenue from orders in range
    ...
    payments_collected = payments_in_range.aggregate(total=Sum("amount"))[
        "total"
    ] or Decimal("0")
    payment_count = payments_in_range.count()

    # Outstanding balance (from non-cancelled, non-delivered orders)
    active_orders = Order.objects.exclude(status__in=["cancelled", "delivered"])
    outstanding_balance = sum(o.remaining_balance for o in active_orders)
    unpaid_orders = active_orders.filter(balance_amount__gt=0).count()

    # Average order value
    avg_order_value = orders_in_range.exclude(status="cancelled").aggregate(
        avg=Avg("total_price")
    )["avg"] or Decimal("0")
    ...
    popular_garments = (
        ...
        .annotate(order_count=Count("order"), total_revenue=Sum("order__total_price"))
    ...
    tailor_performance = (
        ...
        .annotate(
            total_tasks=Count(
                "assigned_tasks",
                filter=Q(
                    assigned_tasks__order__order_date__date__gte=start_date,
                    assigned_tasks__order__order_date__date__lte=end_date,
                ),
            ),
            approved_count=Count(
                "assigned_tasks",
                filter=Q(
                    assigned_tasks__status="approved",
                    assigned_tasks__order__order_date__date__gte=start_date,
                    assigned_tasks__order__order_date__date__lte=end_date,
                ),
            ),
            total_commission=Sum(
                ...
                filter=Q(
                    commissions__earned_date__date__gte=start_date,
                    commissions__earned_date__date__lte=end_date,
                ),
            ),
        )
        ...
        recent_activity.append(
            {
                ...
                "description": f"{order.order_number} - {order.customer.name}",
        ...
        recent_activity.append(
            {
                ...
                "description": f"{payment.payment_number} - {payment.order.customer.name}",
    ...
    stats = {
        ...
        "order_count": total_orders_in_range,
        "completed_orders": orders_in_range.filter(
    ...
    context = {
        ...
        "tailor_performance": tailor_performance,
    ...
    if request.method == "POST":
        try:
            with transaction.atomic():
                ...
                messages.success(
                    request,
                    f"Order {order.order_number} has been claimed and marked as delivered.",
                )
...
def notification_list(request):
    ...
    notifications = (
        ...
        .select_related("sender", "order", "task")
        .order_by("-created_at")
    )
...
def notification_mark_all_read(request):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("HX-Request"):
            return HttpResponse(
                status=204, headers={"HX-Trigger": "notificationsUpdated"}
            )
...
def notification_delete(request, pk):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("HX-Request"):
            return HttpResponse(
                status=204, headers={"HX-Trigger": "notificationDeleted"}
            )
...
def notification_clear_all(request):
    ...
    if request.method == "POST":
        ...
        if request.headers.get("HX-Request"):
            return HttpResponse(
                status=204, headers={"HX-Trigger": "notificationsCleared"}
            )
...
def notification_count(request):
    ...
    if request.headers.get("HX-Request"):
        if unread_count > 0:
            return HttpResponse(
                f'<span class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">{unread_count if unread_count < 100 else "99+"}</span>'
            )
        return HttpResponse("")
...
def commission_dashboard(request):
    ...
    if hasattr(request.user, "profile") and request.user.profile.is_tailor:
        ...
        commissions = (
            TailorCommission.objects.filter(tailor=request.user)
            .select_related("order", "task")
            .order_by("-earned_date")
        )

        # Summary stats
        total_earned = commissions.aggregate(total=Sum("commission_amount"))[
            "total"
        ] or Decimal("0")
        ...
        week_total = week_commissions.aggregate(total=Sum("commission_amount"))[
            "total"
        ] or Decimal("0")
        ...
        month_total = month_commissions.aggregate(total=Sum("commission_amount"))[
            "total"
        ] or Decimal("0")

        # Pending commissions (tasks approved but order not yet claimed)
        pending_tasks = TailoringTask.objects.filter(
            tailor=request.user, status="approved", commission_paid=False
...
def tailor_commission_report(request):
    ...
    # Get commissions
    commissions = (
        TailorCommission.objects.filter(
            tailor=tailor,
...
def admin_commission_report(request):
    ...
    commissions = TailorCommission.objects.filter(
        earned_date__date__gte=start_date, earned_date__date__lte=end_date
    ).select_related("tailor", "order", "task")

    # Overall summary
    total_commissions = commissions.aggregate(total=Sum("commission_amount"))[
        "total"
    ] or Decimal("0")
    total_orders_value = commissions.aggregate(total=Sum("order_amount"))[
        "total"
    ] or Decimal("0")
    total_tasks = commissions.count()
    ...
    garment_breakdown = (
        commissions.values("garment_type")
        .annotate(
            count=Count("id"),
            total_commission=Sum("commission_amount"),
...
def commission_history(request):
    ...
    # Summary
    summary = commissions.aggregate(total=Sum("commission_amount"), count=Count("id"))
...
def export_sales_report_pdf(request):
    """Export comprehensive sales analytics report as PDF"""
    from datetime import timedelta, datetime
    from django.db.models import Sum, Count, Avg, Q
    from .reports import generate_sales_analytics_report
    ...
    popular_garments = list(
        ...
        .values("name", "order_count", "total_revenue")[:10]
    )
    ...
    top_customers = list(
        ...
        .values("name", "order_count", "total_spent")[:10]
    )

    # Generate PDF
    pdf = generate_sales_analytics_report(
        stats=stats,
        daily_orders=daily_orders,
        popular_garments=popular_garments,
        top_customers=top_customers,
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    filename = f"sales_analytics_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.pdf"
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
...
def export_inventory_report_pdf(request):
    ...
    # Handle date range filters for logs
    start_date_str = request.GET.get("start_date")
    end_date_str = request.GET.get("end_date")
    ...
    inventory_logs = (
        ...
        .select_related("fabric", "accessory")
        .order_by("-created_at")
    )
    ...
    pdf = generate_inventory_report(
        ...
        low_stock_fabrics=low_stock_fabrics,
        low_stock_accessories=low_stock_accessories,
        inventory_logs=list(inventory_logs),
        start_date=start_date,
        end_date=end_date,
        generated_by=request.user.get_full_name() or request.user.username,
    )
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    filename = f"inventory_report_{end_date.strftime('%Y%m%d')}.pdf"
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
...
def export_customer_report_pdf(request):
    """Export comprehensive customer analytics report as PDF"""
    from datetime import timedelta, datetime
    from django.db.models import Sum, Count, Max
    from .reports import generate_customer_report
        ...
        customers_data.append(
            {
                ...
                "last_order": customer.last_order_date.strftime("%m/%d/%Y")
    ...
    # Top spenders
    top_spenders = customers_data[:10]
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    filename = f"customer_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.pdf"
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
...
def export_orders_report_pdf(request):
    """Export comprehensive orders report as PDF"""
    from datetime import timedelta, datetime
    from django.db.models import Sum, Count
    from .reports import generate_orders_report
    ...
    stats = {
        ...
        "total_value": float(
            orders.exclude(status="cancelled").aggregate(total=Sum("total_price"))[
                "total"
            ]
            or 0
        ),
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    filename = f"orders_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.pdf"
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
...
def export_payments_report_pdf(request):
    """Export comprehensive payments report as PDF"""
    from datetime import timedelta, datetime
    from django.db.models import Sum, Count
    from .reports import generate_payments_report
    ...
    stats = {
        ...
        "deposits": payments.filter(payment_type="deposit").count(),
        "balance_payments": payments.filter(payment_type="balance").count(),
        "cash": payments.filter(payment_method="cash").count(),
        "gcash": payments.filter(payment_method="gcash").count(),
        "bank_transfer": payments.filter(payment_method="bank_transfer").count(),
    }

    # Generate PDF
    pdf = generate_payments_report(
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    filename = f"payments_report_{start_date.strftime('%Y%m%d')}_{end_date.strftime('%Y%m%d')}.pdf"
    response["Content-Disposition"] = f'attachment; filename="{filename}"'
...
@login_required
@admin_required
def export_tailor_performance_pdf(request):
    """Export tailor performance report as PDF"""
    from datetime import timedelta, datetime
    from django.db.models import Sum, Count
    from .reports import generate_tailor_performance_report
